...
int main(){
    ...
    while (n_actual != NULL)
    {
        ...
        while(circulo != NULL){
            ...
            //printf("k = %d, m = %d -",kvalor,mvalor);
            borrar_k = guardia_k;
            borrar_m = guardia_m;
            guardia_k = guardia_k->next;
            guardia_m = guardia_m->prev;

            //mostrarhorario(guardia_k);
            //printf("\n");
            if(guardia_k == borrar_m){
                //printf("%d - %d",guardia_k->clave,borrar_m->clave);
                guardia_k = (guardia_k->prev);
                guardia_m = (guardia_m->next);
            }
...
Lista *delete(Lista *lista,Lista *borrar){
    ...
    if((devolver->prev)==(devolver->next)){
        return NULL;
    }
